        ...
        function renderFileList(filter = "") {
            ...
            files.filter(f => f.toLowerCase().includes(filter.toLowerCase())).forEach(file => {
                ...
                if (actionType === 'download') {
                    ...
                    item.innerHTML = `<span>${file}</span> <form method="post" action="/cheapify/delete/${encodeURIComponent(file)}" style="display:inline;"><button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Delete this file?');">Delete</button></form>`;
                }
